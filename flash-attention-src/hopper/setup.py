...
if not USE_TRITON_ROCM:
    cutlass_root = os.path.abspath(os.path.join(this_dir, "..", "csrc", "cutlass"))
    cutlass_header = os.path.join(cutlass_root, "include", "cutlass", "cutlass.h")
    if not os.path.exists(cutlass_header):
        source_root = os.path.abspath(os.path.join(this_dir, ".."))
        if os.path.isdir(os.path.join(source_root, ".git")):
            subprocess.run(
                ["git", "-C", source_root, "submodule", "update", "--init", "csrc/cutlass"],
                check=True,
            )
        else:
            raise RuntimeError(
                "csrc/cutlass is missing; use the bundled source distribution "
                "or initialize the upstream CUTLASS submodule"
            )
