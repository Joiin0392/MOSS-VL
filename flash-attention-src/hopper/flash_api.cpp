...
inline void check_cross_kv_boundary(
    ...
    if (!is_varlen_q) {
        ...
        TORCH_CHECK(cross_kv_boundary.dim() == 1,
...
mha_fwd_get_scheduler_metadata(
    ...
    if (cross_kv_boundary_.has_value()) {
        ...
        TORCH_CHECK(!params.is_causal, "cross_kv_boundary (staircase mask) cannot be used together with is_causal");
