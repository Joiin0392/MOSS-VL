...
inline void check_cross_kv_boundary(
        const Tensor& cross_kv_boundary,
        bool is_varlen_q,
        int64_t batch_size,
        int64_t seqlen_q,
        int64_t total_q,
        int device_index) {
    STD_TORCH_CHECK(
        cross_kv_boundary.scalar_type() == torch::headeronly::ScalarType::Int,
        "cross_kv_boundary must have dtype torch.int32");
    CHECK_DEVICE(cross_kv_boundary);
    CHECK_CONTIGUOUS(cross_kv_boundary);
    STD_TORCH_CHECK(cross_kv_boundary.get_device() == device_index,
                    "cross_kv_boundary must be on the same CUDA device as q");
    if (!is_varlen_q) {
        CHECK_SHAPE(cross_kv_boundary, batch_size, seqlen_q);
    } else if (total_q >= 0) {
        CHECK_SHAPE(cross_kv_boundary, total_q);
    } else {
        STD_TORCH_CHECK(cross_kv_boundary.dim() == 1,
                        "cross_kv_boundary must be 1D when cu_seqlens_q is provided");
    }
}
} // anonymous namespace
...
mha_fwd_get_scheduler_metadata(
    ...
    params.window_size_right = window_size_right;
    params.attention_chunk = attention_chunk;
    if (cross_kv_boundary_.has_value()) {
        auto cross_kv_boundary = cross_kv_boundary_.value();
        check_cross_kv_boundary(
            cross_kv_boundary, is_varlen_q, batch_size, max_seqlen_q,
            -1, seqused_k.get_device());
        STD_TORCH_CHECK(!params.is_causal, "cross_kv_boundary (staircase mask) cannot be used together with is_causal");
        params.cross_kv_boundary = static_cast<int const *>(cross_kv_boundary.data_ptr());
        if (!params.is_local) {
...
mha_fwd(Tensor q,   // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
    ...
    if (cross_kv_boundary_.has_value()) {
        auto cross_kv_boundary = cross_kv_boundary_.value();
        check_cross_kv_boundary(
            cross_kv_boundary, is_varlen_q, batch_size, seqlen_q,
            total_q, q.get_device());
        STD_TORCH_CHECK(!params.is_causal, "cross_kv_boundary (staircase mask) cannot be used together with is_causal");
        params.cross_kv_boundary = static_cast<int const *>(cross_kv_boundary.data_ptr());
        if (!params.is_local) {
            params.is_local = true;
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> mha_bwd(
    ...
    bool const is_local = ((window_size_left >= 0 || window_size_right >= 0) && !is_causal) || cross_kv_boundary_.has_value();
    ...
    if (cross_kv_boundary_.has_value()) {
        check_cross_kv_boundary(
            cross_kv_boundary_.value(), is_varlen_q, batch_size, seqlen_q,
            total_q, q.get_device());
    }
...
void boxed_mha_bwd(
    ...
    auto cross_kv_boundary = to<std::optional<Tensor>>(stack[22]);

    auto [softmax_d, softmax_lse_log2, dq_accum, dk_accum, dv_accum] = mha_bwd(dout, q, k, v, out, softmax_lse, dq, dk, dv, cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k, max_seqlen_q, max_seqlen_k, softmax_scale, is_causal, window_size_left, window_size_right, softcap, deterministic, sm_margin, cross_kv_boundary);
...
void boxed_mha_fwd_get_scheduler_metadata(
    ...
    auto cross_kv_boundary = to<std::optional<Tensor>>(stack[24]);

    auto scheduler_metadata = mha_fwd_get_scheduler_metadata(batch_size, max_seqlen_q, max_seqlen_k, num_heads, num_heads_k, headdim, headdim_v, qkv_dtype, seqused_k, cu_seqlens_q, cu_seqlens_k, cu_seqlens_k_new, seqused_q, leftpad_k, page_size, max_seqlen_k_new, is_causal, window_size_left, window_size_right, attention_chunk, has_softcap, num_splits, pack_gqa, sm_margin, cross_kv_boundary);
...
STABLE_TORCH_LIBRARY(flash_attn_3, m) {
    m.def("fwd("
        ...
        "bool? pack_gqa = None,"
        "int sm_margin = 0,"
        "Tensor? cross_kv_boundary = None) -> (Tensor(out!), Tensor, Tensor, Tensor)");
    m.def("bwd("
        "Tensor dout,"
        "Tensor q,"
        "Tensor k,"
        ...
        "bool deterministic = False,"
        "int sm_margin = 0,"
        "Tensor? cross_kv_boundary = None) -> (Tensor, Tensor, Tensor, Tensor, Tensor)");
    m.def("fwd_combine("
        "Tensor out_partial,"
        "Tensor lse_partial,"
        "Tensor(out!)? out = None,"
    ...
    m.def("get_scheduler_metadata("
        ...
        "bool? pack_gqa = None,"
        "int sm_margin = 0,"
        "Tensor? cross_kv_boundary = None) -> Tensor");
}
