...
Usage::
    ...
    from npu_utils import get_device, synchronize, get_default_attn_impl
...
def resolve_attn_impl(requested: str | None = None) -> str:
    ...
    if requested == "flash_attention_2" and not _HAS_CUDA:
        print(
            f"[npu_utils] flash_attention_2 is not available on "
...
def print_device_info() -> None:
    ...
    print(f"[npu_utils] device_type={dt}  device_count={count}")
    for i in range(min(count, 4)):
        print(f"[npu_utils]   device {i}: {get_device_name(i)}")
    print(f"[npu_utils] default attn_impl={get_default_attn_impl()}")
...
def install_flash_attn_stub() -> None:
    ...
    try:
        ...
        print("[npu_utils] Installed flash_attn stub (real package not found).")
